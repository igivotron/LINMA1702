...
def mod3ContSup(R, c, T):
    ...
    if r != 0:
        A = A[:, :-r]
    # C :matrice [p x p*T] pour additionner les productions d'une période
    ...
    D = np.abs(np.dot(P, B))
    # On addictionne tous les deltaP :
    S = np.ones((D.shape[1]))
    F = np.dot(D,S)/(p-1)
    return F
...
def modele3(t, m, n, h, ct, cm, c, Rt, Rm, R, A, P, k, delta, T, checkpoints):
    ...
    if checkpoints:
        print("Checking construction of A_ub : is A_ub2 shape correct ?", np.shape(A_ub2) == (h, n + 1))
    A_ub3 = np.array([np.append(mod3ContSup(R, c, T), 0)])
    print(A_ub3)
    A_ub = np.concatenate((A_ub1, A_ub2, A_ub3))
    if checkpoints:
        print("Checking construction of A_ub : is A_ub shape correct ?", np.shape(A_ub) == (2 * n + h, n + 1))
    b_ub1 = np.ones((n))
    b_ub2 = np.zeros((n+h))
    b_ub3 = np.array([delta*P*T])
